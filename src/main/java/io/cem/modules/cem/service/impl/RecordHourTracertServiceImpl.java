...
public class RecordHourTracertServiceImpl implements RecordHourTracertService {
	...
	@Override
	@Async
	public Future<List<RecordHourTracertEntity>> queryTargetHourList(Map<String, Object> map) {
		return new AsyncResult<>(recordHourTracertDao.queryTargetHourList(map));
	}
	...
	@Override
	public List<ScoreEntity> calculateTarget0(List<ScoreEntity> connection, List<ScoreEntity> quality, List<ScoreEntity> broswer, List<ScoreEntity> download, List<ScoreEntity> video, List<ScoreEntity> game){
		List<ScoreEntity> finalScore = new ArrayList<>();
		Map<ScoreTargetRankEntity,Map<String,ScoreBaseEntity>> score= new HashMap<>();

		for (int i = 0; i < connection.size(); i++) {
			ScoreTargetRankEntity targetRank = new ScoreTargetRankEntity();
			targetRank.setCityId(connection.get(i).getCityId());
			targetRank.setCountyId(connection.get(i).getCountyId());
			targetRank.setProbeId(connection.get(i).getProbeId());
			targetRank.setTargetId(connection.get(i).getTargetId());
			targetRank.setCityName(connection.get(i).getCityName());
			targetRank.setCountyName(connection.get(i).getCountyName());
			targetRank.setProbeName(connection.get(i).getProbeName());
			targetRank.setTargetName(connection.get(i).getTargetName());
			targetRank.setRecordDate(connection.get(i).getRecordDate());
			targetRank.setRecordTime(connection.get(i).getRecordTime());
			targetRank.setAccessLayer(connection.get(i).getAccessLayer());
			targetRank.setPort(connection.get(i).getPort());
			ScoreBaseEntity scoreBase = new ScoreBaseEntity();
			scoreBase.setPingIcmpDelay(connection.get(i).getPingIcmpDelay());
			scoreBase.setPingIcmpDelayStd(connection.get(i).getPingIcmpDelayStd());
			scoreBase.setPingIcmpDelayVar(connection.get(i).getPingIcmpDelayVar());
			...
			Map<String,ScoreBaseEntity> ping1 = new HashMap<>();
			ping1.put("connection",scoreBase);
			score.put(targetRank,ping1 );
		}

		score=putTargetMap(quality,score,"quality");
		score=putTargetMap(broswer,score,"broswer");
		score=putTargetMap(download,score,"download");
		score=putTargetMap(video,score,"video");
		score=putTargetMap(game,score,"game");

		System.out.println("MAP:"+score);

		Set<ScoreTargetRankEntity> key = score.keySet();
	...
	public Map<ScoreTargetRankEntity,Map<String,ScoreBaseEntity>> putTargetMap(List<ScoreEntity> list,Map<ScoreTargetRankEntity,Map<String,ScoreBaseEntity>> map,String type){
		for (int i = 0; i < list.size(); i++) {
			ScoreTargetRankEntity targetRank = new ScoreTargetRankEntity();
